...
const imagePath = path.join(__dirname, '..', 'public', 'images', 'FotoSecretaFinal.png');
const outputPath = path.join(__dirname, '..', 'public', 'images', 'FotoSecretaFinal.jpg');

async function addMetadata() {
  try {
    // Verificar se o PNG existe
    if (!fs.existsSync(imagePath)) {
      console.log('⚠️  FotoSecretaFinal.png não encontrada. Saltando...');
      return;
    }
    ...
    // Não falha o npm install se houver erro
    process.exit(0);
  }
}
